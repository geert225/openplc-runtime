 ...
 */
static int pb_data_type_expected_iec_size(pb_data_type_t dt)
{
    switch (dt) {
    case PB_DTYPE_BOOL:                       return (int)IEC_SIZE_BIT;
    case PB_DTYPE_INT8:   case PB_DTYPE_UINT8: return (int)IEC_SIZE_BYTE;
    case PB_DTYPE_INT16:  case PB_DTYPE_UINT16: return (int)IEC_SIZE_WORD;
    case PB_DTYPE_INT32:  case PB_DTYPE_UINT32:
    case PB_DTYPE_REAL32:                     return (int)IEC_SIZE_DWORD;
    case PB_DTYPE_INT64:  case PB_DTYPE_UINT64:
    case PB_DTYPE_REAL64:                     return (int)IEC_SIZE_LWORD;
    case PB_DTYPE_UNKNOWN:
    case PB_DTYPE_PAD:                        return -1;
    }
    return -1;
}
                             ...
                             plugin_runtime_args_t *args, plugin_logger_t *logger)
{
    ...
    for (int c = 0; c < slave->channel_count; c++) {
        const pb_channel_t *ch = &slave->channels[c];

        /* Skip channels without an IEC location (e.g. pure DPV1 parameters) */
        if (ch->iec_location[0] == '\0')
            continue;

        iec_location_t iec_loc;
        if (pb_io_parse_iec_location(ch->iec_location, &iec_loc) != 0) {
            plugin_logger_warn(logger,
                "Slave '%s' channel '%s': invalid IEC location '%s', skipping",
                slave->name, ch->name, ch->iec_location);
            errors++;
            continue;
        }
        ...
        if (iec_loc.byte_index >= args->buffer_size) {
            plugin_logger_warn(logger,
                ...
                slave->name, ch->name, ch->iec_location, iec_loc.byte_index,
                args->buffer_size);
            errors++;
            continue;
        }

        /* Bounds check against the slave's cyclic data buffer */
        int data_len = (iec_loc.direction == IEC_DIR_INPUT) ? slave->input_length
                                                             : slave->output_length;
        if (ch->byte_offset < 0 || ch->byte_offset >= data_len) {
            plugin_logger_warn(logger,
                "Slave '%s' channel '%s': byte_offset %d exceeds %s data length %d, skipping",
                slave->name, ch->name, ch->byte_offset,
        ...
        int expected_size = pb_data_type_expected_iec_size(ch->data_type);
        if (expected_size >= 0 && expected_size != (int)iec_loc.size) {
            plugin_logger_warn(logger,
                ...
                slave->name, ch->name, pb_data_type_to_string(ch->data_type),
                iec_size_name((iec_size_t)expected_size), ch->iec_location,
                iec_size_name(iec_loc.size));
        }

        pb_channel_map_entry_t entry;
        ...
        if (iec_loc.direction == IEC_DIR_INPUT) {
            if (map->input_count < PB_MAX_MAP_ENTRIES) {
            ...
            if (map->output_count < PB_MAX_MAP_ENTRIES) {
        ...
        plugin_logger_debug(logger,
            "  Mapped: slave '%s' ch '%s' [%s] (%s) -> %s data_byte=%d data_bit=%d",
            slave->name, ch->name, pb_data_type_to_string(ch->data_type), ch->iec_location,
            (iec_loc.direction == IEC_DIR_INPUT) ? "INPUT" : "OUTPUT", ch->byte_offset,
            ch->bit_offset);
    }

    plugin_logger_info(logger, "Slave '%s': channel map built: %d inputs, %d outputs (%d errors)",
                        slave->name, map->input_count, map->output_count, errors);

    /* Reject partial maps: a typo in the JSON would leave the PLC running
     * with stale variables and no indication of why. */
    if (errors > 0) {
        plugin_logger_error(logger, "Slave '%s': channel map rejected: %d entry/entries failed",
                             slave->name, errors);
        return -1;
    }
 ...
 */
static void *resolve_plc_ptr(const pb_channel_map_entry_t *e, iec_dir_t direction,
                               ...
                               plugin_runtime_args_t *args, plugin_logger_t *logger)
{
    ...
    /* Profibus DP input data is published into the %I image through the
    ...
    if (map->input_count > 0 &&
        ...
        plugin_logger_error(logger,
            "Journal write entry points unavailable; cannot map %d Profibus input channel(s)",
            map->input_count);
        return -1;
    }
    ...
    for (int i = 0; i < map->input_count; i++) {
        const pb_channel_map_entry_t *e = &map->inputs[i];
        void *plc_ptr = resolve_plc_ptr(e, IEC_DIR_INPUT, args);
        if (!plc_ptr)
            continue;

        pb_transfer_entry_t *t = &xfer->inputs[xfer->input_count++];
        t->plc_ptr          = plc_ptr;
    ...
    for (int i = 0; i < map->output_count; i++) {
        const pb_channel_map_entry_t *e = &map->outputs[i];
        void *plc_ptr = resolve_plc_ptr(e, IEC_DIR_OUTPUT, args);
        if (!plc_ptr)
            continue;

        pb_transfer_entry_t *t = &xfer->outputs[xfer->output_count++];
        t->plc_ptr          = plc_ptr;
        t->data_byte_offset = e->data_byte_offset;
        t->data_bit_offset  = e->data_bit_offset;
        t->byte_count       = iec_size_to_bytes(e->size);
...
#define PB_JOURNAL_BOOL_INPUT 0
#define PB_JOURNAL_BYTE_INPUT 3
#define PB_JOURNAL_INT_INPUT  5
#define PB_JOURNAL_DINT_INPUT 8
#define PB_JOURNAL_LINT_INPUT 11

void pb_io_read_inputs_fast(const pb_transfer_list_t *xfer, const uint8_t *input_data,
                             plugin_runtime_args_t *args)
{
    for (int i = 0; i < xfer->input_count; i++) {
        const pb_transfer_entry_t *e = &xfer->inputs[i];
        const uint8_t *src = input_data + e->data_byte_offset;

        if (e->is_bit) {
            int v = data_read_bit(src, e->data_bit_offset);
            args->journal_write_bool(PB_JOURNAL_BOOL_INPUT, e->journal_index, e->journal_bit, v);
            continue;
        }

        /* Multi-byte Profibus DP data is copied byte-for-byte into the IEC
         * value. Byte order depends on the slave's GSD (Profibus commonly
         * uses big-endian "Motorola" order for analog channels); adjust
         * channel byte_offset/ordering in the JSON config to match. */
        switch (e->byte_count) {
        case 1: {
            uint8_t v;
            memcpy(&v, src, 1);
            args->journal_write_byte(PB_JOURNAL_BYTE_INPUT, e->journal_index, v);
            break;
        }
        case 2: {
            uint16_t v;
            memcpy(&v, src, 2);
            args->journal_write_int(PB_JOURNAL_INT_INPUT, e->journal_index, v);
            break;
        }
        case 4: {
            uint32_t v;
            memcpy(&v, src, 4);
            args->journal_write_dint(PB_JOURNAL_DINT_INPUT, e->journal_index, v);
            break;
        }
        case 8: {
            uint64_t v;
            memcpy(&v, src, 8);
            args->journal_write_lint(PB_JOURNAL_LINT_INPUT, e->journal_index, v);
            break;
        }
        default:
            break;
...
void pb_io_write_outputs_fast(const pb_transfer_list_t *xfer, uint8_t *output_data)
{
    for (int i = 0; i < xfer->output_count; i++) {
        const pb_transfer_entry_t *e = &xfer->outputs[i];
